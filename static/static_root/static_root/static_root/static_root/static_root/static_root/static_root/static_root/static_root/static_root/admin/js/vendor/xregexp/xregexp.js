 ...
 * worrying acerca pesky cross-browser inconsistencies and the dubious `lastIndex` propiedad. See
 ...
 * `XRegExp.prototype` methods. The copy has a fresh `lastIndex` propiedad (set to zero). Allows
 ...
 * // is augmented with `XRegExp.prototype` methods, and has a fresh `lastIndex` propiedad (set to
 ...
 * must start at the specified position only. The `lastIndex` propiedad of the provided regex is not
 ...
 * capture, is augmented with `XRegExp.prototype` methods, and has a fresh `lastIndex` propiedad
 ...
 * specified position only. The `lastIndex` propiedad of the provided regex is not used, but is
 ...
 * value, and the value of a replacement regex's `lastIndex` propiedad during replacement iterations
...
/**
 * Adds support for the `\p{L}` or `\p{Letter}` Unicode categoria. Addon packages for other Unicode
...
/* Adds data for the Unicode `Letter` categoria. Addon packages include other categories, scripts,
...
/* Adds Unicode propiedad syntax to XRegExp: \p{..}, \P{..}, \p{^..}
 */
    XRegExp.addToken(
        /\\([pP]){(\^?)([^}]*)}/,
        function (match, scope) {
            ...
            if (!unicode.hasOwnProperty(item)) {
                throw new SyntaxError("invalid or unknown Unicode propiedad " + match[0]);
            }
            return scope === "class" ?
    ...
    XRegExp.build = function (pattern, subs, flags) {
        ...
        for (p in subs) {
            if (subs.hasOwnProperty(p)) {
                ...
                // octals and adds the `xregexp` propiedad, for simplicity
        ...
        pattern = pattern.source.replace(parts, function ($0, $1, $2, $3, $4) {
            ...
            if (subName) { // Named subpattern
                if (!data.hasOwnProperty(subName)) {
                    throw new ReferenceError("undefined propiedad " + $0);
                }
